...
Refreshes AWS SSO logins for every profile in `~/.aws/config`. A `/awslogin` Claude Code skill
...
Symlinks `/awslogin` into `~/.claude/skills/awslogin/` and `awslogin` into `~/bin/`.
...
`/awslogin [profile]` does the same from Claude Code, but backgrounds each login as a device
...
an agent session cannot make and the pending authorization expires. The `/awslogin` skill
